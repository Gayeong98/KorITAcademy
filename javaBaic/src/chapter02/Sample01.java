...
public class Sample01 {
	...
	public static int getBoardList() {
		// 게시물 목록을 선택하면 작성된 게시물을 모두 보여줌. (게시물 구분자와 게시물 제목, 게시물 작성자만 출력)
	...
	public static void main(String[] args) {
		...
		while(power) {
			...
			int nameSelectNumber = sc.nextInt();
			// 게시물 작성을 선택하면 타이틀, 작성자, 작성 날짜 및 시간, 내용을 입력할 수 있음
			if(nameSelectNumber < 0 || nameSelectNumber > 2) {
				System.out.println("잘못된 번호를 선택하셨습니다. ");
				continue;
			}
			
			if(nameSelectNumber == 1) writeBoard();
			
			if(nameSelectNumber == 2) getBoardList();
			
			if(nameSelectNumber == 0) {
				System.out.println("프로그램 종료");
				power = false;
				// while문의 조건을 false로 바꿈
				// break문 사용-> while문 강제 종료
				// return으로 main메서드를 종료
			}
