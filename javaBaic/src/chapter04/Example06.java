...
class Thread1 extends Thread {
	...
	public void run() {
		for(int i = 0; i < 20; i++) {
			System.out.print("*");
			Thread.yield(); // 자신의 시간을 다음 순번의 thread에 양보한다. 
...
class CountDown extends Thread {
	public void run() {
		// Thread.sleep()는 지정한 시간동안 해당 스레드를 Blocked 상태로 만듦
		for(int i = 10; i > 0; i--) {
			System.out.println(i + "초...");
			
			try {
				// 해당 스레드를 1초동안 Blocked 상태로 만듦
				// blocked상태: 실행 중이지만 CPU 스케줄러에 선택은 받지 못하는 상태
				Thread.sleep(1000);
			} catch (Exception e) {
				e.printStackTrace();
			}
